...
public function updateStatus(Request $request, $id)
{
    ...
    if ($request->status === 'completed') {
        ...
        // Only auto-mark paid if not already handled by processPayment
        // and the order actually has a total (guards against accidental completes)
        if ($order->payment_status === 'unpaid' && $order->total > 0) {
...
public function updateServiceCharge(Request $request, $id)
{
    ...
    /**
     * POST /api/orders/{id}/items/{itemId}/addons
     * Add an add-on to a table order item
     */
    ...
    /**
     * PUT /api/orders/{id}/items/{itemId}/addons/{addonId}
    ...
    /**
     * DELETE /api/orders/{id}/items/{itemId}/addons/{addonId}
