...
class TableController extends Controller
{
    public function index()
    {
        ...
        // Add total revenue to each table (table orders only)
        ...
        // Calculate total revenue (table + direct orders)
    ...
    public function uploadImage(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $filename = 'table_' . time() . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
            
            // Store the image in the tables directory
            $path = $image->storeAs('tables', $filename, 'public');
            
            return response()->json([
                'filename' => $filename,
                'path' => $path,
                'url' => asset('storage/tables/' . $filename)
            ]);
        }
